...
export function StreamPlayer({
  ...
  const meshSessionActive = Boolean(
    useMesh &&
      device &&
      isRealDevice(device) &&
  ...
  const whepEnabled = !useMesh && isStreaming && Boolean(device?.whepUrl) && (!isIpCam || ipStreamKind === 'whep');
  const ipCamEnabled = isIpCam && isStreaming && ipStreamKind != null && ipStreamKind !== 'whep' && ipStreamKind !== 'unsupported';
  const meshEnabled = meshSessionActive && !isIpCam;
  ...
  const showOffline =
    !isStreaming &&
    !isPairedOnline &&
    ((!isIpCam && !useMesh && !device.whepUrl) ||
  ...
  return (
    <div className={cn('relative h-full w-full overflow-hidden bg-black', className)} style={style}>
      ...
      {isPairedOnline && !isStreaming ? (
        <div className="flex h-full flex-col items-center justify-center gap-1 text-mixer-green">
          <span className={cn('font-bold tracking-wider', compact ? 'text-[8px]' : 'text-xs')}>CONNECTED</span>
